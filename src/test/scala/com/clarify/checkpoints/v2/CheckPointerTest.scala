...
import com.clarify.checkpoints.v2.CheckPointer.{__internalCheckpointBucketWithPartitions, checkpointBucketToDisk, checkpointBucketWithPartitions}
...
class CheckPointerTest extends QueryTest with SparkSessionTestWrapper {
  ...
  test("checkpoint multiple rows multiple times") {
    ...
    val bucket_columns: util.ArrayList[String] = new util.ArrayList[String]()
    bucket_columns.add("id")
    bucket_columns.add("v2")

    val location = Files.createTempDirectory("parquet").toFile.toString
    __internalCheckpointBucketWithPartitions(sql_ctx = spark.sqlContext,
