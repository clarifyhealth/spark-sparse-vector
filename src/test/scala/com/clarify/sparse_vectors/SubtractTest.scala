...
class SubtractTest extends QueryTest with SparkSessionTestWrapper {
  ...
  test("subtract with vectors") {
    ...
    val out_df = spark.sql(
      "select sparse_vector_subtract(v1, v2) as result from my_table2"
    )
    ...
    checkAnswer(
      ...
      Seq(
        Row(new SparseVector(3, Array(0, 2), Array(0.0, 0.0))),
        Row(new SparseVector(3, Array(0, 2), Array(0.0, -0.2))),
        Row(new SparseVector(3, Array(0,1,2), Array(0.0, 0.1, -0.5)))
      )
    )
