...
object StatsCalculator {
  ...
  def _create_statistics(
    ...
    Helpers.log(
      f"Calculating histograms for $view columns: $columns_to_histogram"
    )
    // this returns List[column_name, List[(value. value_count)]
    val histogram_data_frames: Seq[(String, DataFrame)] =
      _create_histogram_array(columns_to_histogram, loaded_df)

    val histogram_results: Seq[(String, Array[Row])] =
      histogram_data_frames.par.map(row => (row._1, row._2.collect())).seq

    Helpers.log(
      f"Finished calculating histograms for $view columns: $columns_to_histogram"
    )
    ...
    for (normal_column <- normal_columns) {
      ...
      if (isNumeric(data_type)) {
        ...
        my_result = loaded_df.select(
          ...
          round(
            count(when(col(column_name) < lit(0), lit(1))) * 100 / sample_record_count,
            3
          ).cast(DoubleType)
      ...
      if (columns_to_histogram contains column_name) {
        val histogram_array_tuple: (String, Array[Row]) =
          histogram_results.filter(x => x._1 == column_name).head
        val histogram_array: Array[Row] = histogram_array_tuple._2
        var i: Int = 0
        for (histogram_row: Row <- histogram_array) {
          val histogram_percent
              : Double = histogram_row.getDouble(1) * 100 / sample_record_count
          my_result = my_result
            .withColumn(f"top_value_${i + 1}", lit(histogram_row.getString(0)))
            .withColumn(
              f"top_value_percent_${i + 1}",
              round(lit(histogram_percent), 3)
            )
          i += 1
        }
      }
  ...
  def _create_histogram_array(
      columns_to_histogram: Seq[String],
      loaded_df: DataFrame
  ): Seq[(String, DataFrame)] = {
    var result: Seq[(String, DataFrame)] = Seq()
    for (column_name <- columns_to_histogram) {
      result = result :+ (column_name, _calculate_histogram_array_for_column(
        column_name,
        loaded_df
      ))
    }
  ...
  def create_histogram(
    ...
    val empty_rdd: RDD[Row] =
      loaded_df.sqlContext.sparkContext.makeRDD(Seq[Row]())
    val result_histogram_df: DataFrame = loaded_df.sqlContext.sparkSession
      .createDataFrame(empty_rdd, histogram_schema)

    result_histogram_df
  }

  def _calculate_histogram_array_for_column(
      column_name: String,
      loaded_df: DataFrame
  ): DataFrame = {
