...
object PartitionDiagnostics {
  ...
  def getPartitionsAndCount(sql_ctx: SQLContext, view: String, result_view: String): DataFrame = {
    val loaded_df: DataFrame = sql_ctx.table(view)
    loaded_df.show()
    val my_rdd: RDD[Row] = loaded_df.rdd.mapPartitionsWithIndex(_mapPartition)
